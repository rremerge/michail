...
test("advisor portal home serves html", async () => {
  ...
  try {
    ...
    assert.match(response.body, /id="workspaceClientQuickSearch"/);
    assert.match(response.body, /id="workspaceConnectionsDetailList"/);
    assert.match(response.body, /id="workspaceSettingsDetailList"/);
    assert.match(response.body, /id="workspaceBrandingDetailList"/);
    assert.match(response.body, /id="workspaceUsageDetailList"/);
    assert.match(response.body, /id="workspaceDebugDetailList"/);
    assert.match(response.body, /id="workspaceHintsCard"/);
    assert.match(response.body, /Connect a calendar now/);
    assert.match(response.body, /Import or add clients/);
    ...
    assert.match(response.body, /Connect your first calendar/);
    assert.match(response.body, /--bg-top:/);
    assert.match(response.body, /class="workspace-nav-button"/);
...
test("availability page treats declined client meetings as open time", async () => {
  const tokenId = "declinedmeetingtoken";
  const nowMs = Date.now();
  const handler = createPortalHandler({
    async getAvailabilityLink(tableName, suppliedTokenId) {
      assert.equal(tableName, "AvailabilityLinkTable");
        ...
        clientMeetings: [
          {
            ...
            advisorResponseStatus: "declined"
          }
        ],
        nonClientBusyIntervals: []
      };
  ...
  try {
    ...
    assert.equal(response.statusCode, 200);
    assert.equal(response.body.includes("Declined Meeting"), false);
    assert.equal(response.body.includes("Your meeting (pending)"), false);
    assert.equal(response.body.includes("data-slot-status=\"busy\""), false);
  } finally {
    for (const [key, value] of Object.entries(previousValues)) {
      if (value === undefined) {
        delete process.env[key];
